...
liveRecv = function(data) {
    ...
    if (data.new_score !== undefined) {
        ...
        if (data.new_score > 0) {
            scoreElement.classList.add("fw-bold", "text-success");
        }
    }
    // =========================
    // UPDATE TARGET
    ...
    if (data.new_board !== undefined) {
        ...
        boardBody.innerHTML = ""; // Hanya hapus isi tbody

        data.new_board.forEach(row => {
            const tr = document.createElement("tr");
            row.forEach(char => {
                const td = document.createElement("td");
                td.className = "board-cell border border-3 fs-4";
    ...
    if (data.correct !== undefined) {
        ...
        // Hilangkan feedback setelah 1.5 detik
        setTimeout(() => {
